...
import "./interfaces/IxMOLE.sol";
import "./interfaces/IBEP20.sol";

import "./SafeToken.sol";
import "./utils/Math128.sol";

/// @title GrassHouse - Where Mole eats
...
contract GrassHouse is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  address public xMOLE;
  address public rewardToken;
  uint256 public lastTokenBalance;

  /// @dev xMOLE supply at week bounds
  ...
  /// @param _xMOLE The address of xMOLE
  ...
  /// @notice Get xMOLE balance of "_user" at "_timstamp"
  ...
  function balanceOfAt(address _user, uint256 _timestamp) external view returns (uint256) {
    uint256 _maxUserEpoch = IxMOLE(xMOLE).userPointEpoch(_user);
    if (_maxUserEpoch == 0) {
      return 0;
    }

    uint256 _epoch = _findTimestampUserEpoch(_user, _timestamp, _maxUserEpoch);
    Point memory _point = IxMOLE(xMOLE).userPointHistory(_user, _epoch);
  ...
  function _checkpointTotalSupply() internal {
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      if (_weekCursor > _roundedTimestamp) {
        ...
        Point memory _point = IxMOLE(xMOLE).pointHistory(_epoch);
        int128 _timeDelta = 0;
  ...
  /// @notice Update xMOLE total supply checkpoint
  ...
  function _claim(address _user, uint256 _maxClaimTimestamp) internal returns (uint256) {
    ...
    Point memory _userPoint = IxMOLE(xMOLE).userPointHistory(_user, _userEpoch);
    ...
    // This can be:
    // 1) User just lock their MOLE less than 1 week
    // 2) User already claimed their rewards
    if (_userWeekCursor >= _maxClaimTimestamp) {
      return 0;
    }

    // Handle when user lock MOLE before Grasshouse started
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      ...
      if (_userWeekCursor >= _userPoint.timestamp && _userEpoch <= _maxUserEpoch) {
        ...
        if (_userEpoch > _maxUserEpoch) {
          ...
          _userPoint = IxMOLE(xMOLE).userPointHistory(_user, _userEpoch);
        }
      } else {
  ...
  function _findTimestampEpoch(uint256 _timestamp) internal view returns (uint256) {
    uint256 _min = 0;
    uint256 _max = IxMOLE(xMOLE).epoch();
    ...
    for (uint256 i = 0; i < 128; i++) {
      ...
      Point memory _point = IxMOLE(xMOLE).pointHistory(_mid);
  ...
  function _findTimestampUserEpoch(
    ...
    for (uint256 i = 0; i < 128; i++) {
      ...
      uint256 _mid = (_min + _max + 1) / 2;
      Point memory _point = IxMOLE(xMOLE).userPointHistory(_user, _mid);
      if (_point.timestamp <= _timestamp) {
        _min = _mid;
      } else {
        _max = _mid - 1;

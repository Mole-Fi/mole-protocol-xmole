...
/// @title xMOLE - The goverance token of Mole 
// solhint-disable not-rely-on-time
// solhint-disable-next-line contract-name-camelcase
contract xMOLE is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  /// @dev Token to be locked (MOLE)
  address public token;
  /// @dev Total supply of MOLE that get locked
  ...
  /// @notice Initialize xMOLE
  /// @param _token The address of MOLE token
  ...
  /// @notice Return the balance of xMOLE at a given "_blockNumber"
  /// @param _user The address to get a balance of xMOLE
  /// @param _blockNumber The speicific block number that you want to check the balance of xMOLE
    ...
    if (_address != address(0)) {
      ...
      if (_newLocked.end != 0) {
        ...
        if (_newLocked.end == _prevLocked.end) {
          ...
          // Possibly when user deposited more MOLE to his locker
  ...
  /// @dev This will crate a new lock and deposit MOLE to xMOLE Vault
  /// @param _amount the amount that user wishes to deposit
  /// @param _unlockTime the timestamp when MOLE get unlocked, it will be
  ...
  /// @notice Internal function to perform deposit and lock MOLE for a user
  /// @param _for The address to be locked and received xMOLE
  /// @param _amount The amount to deposit
  /// @param _unlockTime New time to unlock MOLE. Pass 0 if no change.
  ...
  /// @param _amount The amount of MOLE to be added to the lock
  ...
  /// @notice Calculate total supply of xMOLE (voting power)
  function totalSupply() external view returns (uint256) {
    return _totalSupplyAt(pointHistory[epoch], block.timestamp);
  }

  /// @notice Calculate total supply of xMOLE at specific block
  ...
  /// @notice Calculate total supply of xMOLE (voting power) at some point in the past
  ...
  /// @notice Withdraw all MOLE when lock has expired.

...
/// @title MoleFeeder
contract MoleFeeder02 is IVault, Initializable, OwnableUpgradeable {
  /// @notice Libraries
  using SafeToken for address;

  /// @notice Errors
  error MoleFeeder02_InvalidToken();
  error MoleFeeder02_InvalidRewardToken();
  error MoleFeeder02_Deposited();
  ...
  function initialize(
    ...
    if (_stakeToken != _proxyToken) revert MoleFeeder02_InvalidToken();
    if (grassHouse.rewardToken() != _token) revert MoleFeeder02_InvalidRewardToken();
  ...
  function miniFLDeposit() external onlyOwner {
    if (IBEP20(proxyToken).balanceOf(address(miniFL)) != 0) revert MoleFeeder02_Deposited();
